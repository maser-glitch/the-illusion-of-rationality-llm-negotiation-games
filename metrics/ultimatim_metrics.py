...
from metrics_utils import ultimatum_logs_to_df
...
df = ultimatum_logs_to_df("ultimatum-experiment")

df["models"] = df["player_one_model"] + "@" + df["player_two_model"]
target_cols = ["player_one_win", "player_two_win",
               "draw", "player_one_delta", "player_two_delta"]
...
df_grouped['player_one_win_rate'] = df_grouped['player_one_win'].divide(win_sum).fillna(0)
df_grouped['player_two_win_rate'] = df_grouped['player_two_win'].divide(win_sum).fillna(0)


win_rate_data = df_grouped.pivot(index="model_2",
                                 columns="model_1",
                                 values="player_one_win_rate")
win_rate_data = win_rate_data.rename(index=NAMES_MAP,
                                     columns=NAMES_MAP)

payoff_data = df_grouped.pivot(index="model_2",
                               columns="model_1",
                               values="player_one_delta")
...
axes[0].set_ylabel('Responser')
axes[0].set_xlabel('Proposer')
axes[0].set_title('Proposer Win Rate')
...
axes[1].set_ylabel('')
axes[1].set_xlabel('Proposer')
axes[1].set_title('Proposer Average Payoff')
...
fig.savefig(out_dir / "ultimatum_metrics.pdf", dpi=300, bbox_inches='tight')
