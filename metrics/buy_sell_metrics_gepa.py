...
df_opti = buy_sell_logs_to_df(log_name="buysell-experiment-optimized")

df_opti["seller_suffix"] = df_opti["seller_social_behavior"].apply(
    lambda behavior: "_optimized" if behavior else ""
)
df_opti["buyer_suffix"] = df_opti["buyer_social_behavior"].apply(
    lambda behavior: "_optimized" if behavior else ""
)

df_opti["seller_model"] = df_opti["seller_model"] +  df_opti["seller_suffix"]
df_opti["buyer_model"] = df_opti["buyer_model"] +  df_opti["buyer_suffix"]

df = pd.concat([df, df_opti])
...
model_colors = {
    "gemini-2.5-pro": "#7f7f7f",
    "gemini-2.5-flash": "#7f7f7f",
    "gpt-4.1": "#7f7f7f",
    "gpt-4.1-mini": "#76DECD",
    "gpt-4o": "#7f7f7f",
    "sonnet4.5": "#7f7f7f",
    "gpt-4.1-mini-optimized": "#76DECD",
}
...
df["buyer_model_short"]  = df["buyer_model"].map(
    model_map
).fillna(df["buyer_model"])
df["seller_model_short"] = df["seller_model"].map(
    model_map
).fillna(df["seller_model"])
...
plt.savefig(plots_dir / f"{log_name}_scatter_gepa.pdf", dpi=300,
            bbox_inches='tight')
